...
<form class="form-inline" method="POST" action="<?=$this->base?>/Ateninfcurativas/index">
...
<table class="table table-bordered">
  ...
  if(ISSET($_POST['search'])){
      $date1 = $_POST['date1'];
      $query=mysqli_query($conectar, "SELECT * FROM `ateninfcurativas` inner join regiones on regiones.id = ateninfcurativas.regiones_id WHERE `año` = '$date1'") or die(mysqli_error());
      ...
      if($row > 0){
        while($fetch = mysqli_fetch_assoc($query)){
          ?>
          <tr> 
            <td><?php echo $this->Html->link($fetch['nombre'], array('controller' => 'Acinfxestas', 'action' => 'index', $fetch['regiones_id'], $fetch['año'] )); ?></td>
    ...
    <tfoot>
      <tr>
        ...
        if(ISSET($_POST['search'])){
          $date1 = $_POST['date1'];
          $result = mysqli_query($conectar, 'SELECT SUM(trimestre1) AS total, SUM(trimestre2) AS total2, SUM(trimestre3) AS total3, SUM(trimestre4) AS total4 FROM ateninfcurativas where año ='.$date1); 
    ...
    $query=mysqli_query($conectar, "SELECT * FROM `ateninfcurativas` inner join regiones on regiones.id = ateninfcurativas.regiones_id WHERE `año` =2020") or die(mysqli_error());
    while($fetch = mysqli_fetch_array($query)){
      ?>
      <tr>
        <td><?php echo $this->Html->link($fetch['nombre'], array('controller' => 'Acinfxestas', 'action' => 'index', $fetch['regiones_id'], 2020 )); ?></td>
        <td><?php echo $fetch['trimestre1'] ?></td>
        <td><?php echo $fetch['trimestre2'] ?></td>
        <td><?php echo $fetch['trimestre3'] ?></td>
        <td><?php echo $fetch['trimestre4'] ?></td>
  ...
  <tr>
    <td> Total </td>
    <?php
      $result = mysqli_query($conectar, 'SELECT SUM(trimestre1) AS total, SUM(trimestre2) AS total2, SUM(trimestre3) AS total3, SUM(trimestre4) AS total4 FROM ateninfcurativas where año = 2020'); 
      $row = mysqli_fetch_assoc($result); 
      $suma = ($row['total'] + $row['total2'] + $row['total3'] + $row['total4']);
    ?>
    <th><?php echo $row['total']; ?></th>
