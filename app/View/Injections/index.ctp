...
<h2><center>Inyecciones</center></h2>

<?= $this->Form->create('injections', ['type' => 'get']); ?>
    ...
    <table class="table table-bordered table-condensed" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($injections as $injection) : ?>
                <tr>
                    <td><?php echo h($injection['Injection']['id']); ?>&nbsp;</td>
                    <td>
                        <?php $region = $injection['Region']['id']; ?>
                        <?php echo $this->Html->link($injection['Region']['region_name'], array('controller' => 'Injxestablishments', 'action' => 'index', $region, $yer)); ?>
                    </td>
                    <?php $total = $injection['Injection']['trimester1'] + $injection['Injection']['trimester2'] + $injection['Injection']['trimester3'] + $injection['Injection']['trimester4'] ?>
                    <td><?php echo h($injection['Injection']['trimester1']); ?>&nbsp;</td>
                    <td><?php echo h($injection['Injection']['trimester2']); ?>&nbsp;</td>
                    <td><?php echo h($injection['Injection']['trimester3']); ?>&nbsp;</td>
                    <td><?php echo h($injection['Injection']['trimester4']); ?>&nbsp;</td>
                    <td><?php echo $total; ?>&nbsp;</td>
                    <td><?php echo h($injection['Injection']['year']); ?>&nbsp;</td>
                    <td class="actions">
                        <?php echo $this->Html->link(__('View'), array('action' => 'view', $injection['Injection']['id'])); ?>
                        <?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $injection['Injection']['id'])); ?>
                        <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $injection['Injection']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $injection['Injection']['id']))); ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<div class="actions">
    ...
    <ul>
        ...
        <li><?php echo $this->Html->link(__('New Injection'), array('action' => 'add')); ?></li>
    </ul>
</div>
<script>
