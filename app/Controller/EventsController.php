...
class EventsController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($yir) {
            ...
            // Event.year debe ser cambiado al año actual, igual que en el filtro
            $trim1 = $this->Event->find('all', array(
                'fields' => array('SUM(Event.trimester1) as t1, SUM(Event.trimester2) as t2, SUM(Event.trimester3) as t3, SUM(Event.trimester4) as t4'),
    ...
    public function chart($yer)
    {
        ...
        $reg = $this->Event->find('all', array(
            'fields' => array('Event.trimester1 as t1, Event.trimester2 as t2, Event.trimester3 as t3, Event.trimester4 as t4'),
            'conditions' => array('Event.year =' => $yer, 'Event.regions_id =' => 1)
        ));

        $reg2 = $this->Event->find('all', array(
            'fields' => array('Event.trimester1 as t1, Event.trimester2 as t2, Event.trimester3 as t3, Event.trimester4 as t4'),
            'conditions' => array('Event.year =' => $yer, 'Event.regions_id =' => 2)
        ));
        $reg3 = $this->Event->find('all', array(
            'fields' => array('Event.trimester1 as t1, Event.trimester2 as t2, Event.trimester3 as t3, Event.trimester4 as t4'),
            'conditions' => array('Event.year =' => $yer, 'Event.regions_id =' => 3)
        ));
        $reg4 = $this->Event->find('all', array(
            'fields' => array('Event.trimester1 as t1, Event.trimester2 as t2, Event.trimester3 as t3, Event.trimester4 as t4'),
            'conditions' => array('Event.year =' => $yer, 'Event.regions_id =' => 4)
        ));
        $reg5 = $this->Event->find('all', array(
            'fields' => array('Event.trimester1 as t1, Event.trimester2 as t2, Event.trimester3 as t3, Event.trimester4 as t4'),
            'conditions' => array('Event.year =' => $yer, 'Event.regions_id =' => 5)
        ));

        $tot_occ_1 = $reg[0]['Event']['t1'];
        $tot_occ_2 = $reg[0]['Event']['t2'];
        $tot_occ_3 = $reg[0]['Event']['t3'];
        $tot_occ_4 = $reg[0]['Event']['t4'];

        $tot_cen_1 = $reg2[0]['Event']['t1'];
        $tot_cen_2 = $reg2[0]['Event']['t2'];
        $tot_cen_3 = $reg2[0]['Event']['t3'];
        $tot_cen_4 = $reg2[0]['Event']['t4'];

        $tot_met_1 = $reg3[0]['Event']['t1'];
        $tot_met_2 = $reg3[0]['Event']['t2'];
        $tot_met_3 = $reg3[0]['Event']['t3'];
        $tot_met_4 = $reg3[0]['Event']['t4'];

        $tot_par_1 = $reg4[0]['Event']['t1'];
        $tot_par_2 = $reg4[0]['Event']['t2'];
        $tot_par_3 = $reg4[0]['Event']['t3'];
        $tot_par_4 = $reg4[0]['Event']['t4'];

        $tot_ori_1 = $reg5[0]['Event']['t1'];
        $tot_ori_2 = $reg5[0]['Event']['t2'];
        $tot_ori_3 = $reg5[0]['Event']['t3'];
        $tot_ori_4 = $reg5[0]['Event']['t4'];
        //**********************************
