...
class TCPDF {
	 ...
	 * @public
	 */
	public function setHeaderData($ln='',$ln2='', $lw=0, $ht='', $hs='', $tc=array(0,0,0), $lc=array(0,0,0)) {
		$this->header_logo = $ln;
                $this->header_logo2 = $ln2;
		$this->header_logo_width = $lw;
		//$this->header_logo2_width = 200;
		...
		if ($this->header_xobjid < 0) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				...
				$imgy = $this->y;
                        ...
                        if ($this->rtl) {
				$this->x = $this->w - $this->original_rMargin + 235;
			} else {
                            if($this->CurOrientation=="P")
                                $this->x = $this->original_lMargin + 155;
                                else
				$this->x = $this->original_lMargin + 235;
			}
			if (($headerdata['logo2']) AND ($headerdata['logo2'] != K_BLANK_IMAGE)) {
				$imgtype = TCPDF_IMAGES::getImageFileType(K_PATH_IMAGES.$headerdata['logo2']);
				if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
					$this->ImageEps(K_PATH_IMAGES.$headerdata['logo2'], '', '', $headerdata['logo_width']);
				} elseif ($imgtype == 'svg') {
					$this->ImageSVG(K_PATH_IMAGES.$headerdata['logo2'], '', '', $headerdata['logo_width']);
				} else {
					$this->Image(K_PATH_IMAGES.$headerdata['logo2'], '', '', $headerdata['logo_width']);
				}
				$imgy = $this->getImageRBY();
			} else {
				$imgy = $this->y;
